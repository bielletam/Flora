...
import { today, cn, categoryColor } from "@/lib/utils"
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-3 gap-2.5 mb-4">
        ...
        <StatCard label="Done today" value={doneCount} accent="#0F766E" />
          ...
          <div className={tab !== "all" ? "hidden" : ""}>
            <div className="space-y-4">
              {categories.map((cat) => {
                ...
                const ink = categoryColor(cat).ink
                return (
                  <Card key={cat}>
                    <div className="flex items-center gap-2.5 mb-2">
                      <div className="w-[3px] h-[11px] rounded-[2px]" style={{ background: ink }} />
                      <p className="text-[10.5px] font-bold uppercase tracking-[0.12em]" style={{ color: ink }}>{cat}</p>
...
function HabitRow({ habit, onDelete }: { habit: HabitWithStats; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="grid grid-cols-[1fr_auto_46px_16px] items-center gap-4 py-[11px] rounded-lg hover:bg-surface transition-colors group px-2.5">
      <div className="min-w-0">
        <p className="text-[13px] font-medium text-ink truncate">{habit.name}</p>
        <p className="text-[12.5px] text-[#9A988F] mt-0.5">{habit.frequency} · {habit.rate}% this month</p>
      </div>
      <div className="flex gap-1">
        {habit.dots.slice(-7).map((d, i) => (
          <div key={i} className={cn("w-[7px] h-[7px] rounded-full", d ? "bg-sage" : "bg-[#E7E6E2]")} />
        ))}
      </div>
      <span className="text-[13px] font-semibold text-[#57554E] text-right">{habit.rate}%</span>
      <button
        onClick={deleteHabit}
        className="opacity-0 group-hover:opacity-100 text-ghost hover:text-coral transition-all"
      >
